...
class PRWLoader:
    ...
    def __init__(self, dataset_root, split, gallery_size=100):
        ...
        self.id_mapper = self.setup_id_mapper(split)
        self.split = split
        self.images_root = osp.join(self.dataset_root, "frames")
        self.images = self.load_images()
        self.roidb = self.load_roidb()
        if split == "test":
            self.probes = self.load_probes()

    def setup_id_mapper(self, split):
        if split == "train":
            id_array = loadmat(osp.join(self.dataset_root, "ID_train.mat"))[
                "ID_train"].squeeze()
        elif split == "test":
            id_array = loadmat(osp.join(self.dataset_root, "ID_test.mat"))[
                "ID_test2"].squeeze()
        else:
            raise KeyError("split should in {'train', 'test'}")

        id_mapper = dict()
        for new, ori in enumerate(id_array):
            id_mapper[ori] = new
        return id_mapper

    def map_id(self, idx):
        '''
        map dataset id to training/testing id
        '''
        if isinstance(idx, int):
            if idx in self.id_mapper.keys():
                return self.id_mapper[idx]
            else:
                return -1
            ...
            mapped = []
            for each in idx:
                mapped.append(self.map_id(int(each)))
    ...
    def load_images(self):
        ...
        # Test images
        if self.split == "train":
            train = loadmat(osp.join(self.dataset_root, "frame_train.mat"))[
                "img_index_train"].squeeze()
            train = [each.item() + ".jpg" for each in train]
            return train
        elif self.split == "test":
            test = loadmat(osp.join(self.dataset_root, "frame_test.mat"))[
                "img_index_test"].squeeze()
            test = [each.item() + ".jpg" for each in test]
            return test
        else:
            raise RuntimeError("split should one of ['train', 'test']")

    def load_probes(self):
        """Load the list of (img, roi) for probes."""
        with open(osp.join(self.dataset_root, "query_info.txt")) as f:
            lines = f.readlines()
        lines = [each.strip().split() for each in lines]
        probes = []
        for item in lines:
            im_name = osp.join(self.images_root, item[5] + ".jpg")
            roi = np.asarray(list(map(float, item[1:5])))
            roi[2:] += roi[:2]
    ...
    def load_annotations(self, filename):
        anno = loadmat(osp.join(self.dataset_root,
                                "annotations", filename + ".mat"))
        key = list(set(anno.keys()) -
                   {"__header__", "__version__", "__globals__"})[0]
        return anno[key].astype(np.int32)
    ...
    def load_roidb(self):
        ...
        # if osp.isfile(cache_misc) and osp.isfile(cache_file):
        ...
        for i, im_name in enumerate(self.images):
            anno = self.load_annotations(im_name)
            boxes = anno[:, 1:]
            boxes[:, 2:] += boxes[:, :2]
            pids = self.map_id(anno[:, 0].reshape(-1))
        ...
        self.cls_num = len(self.id_mapper.keys())
...
class PRW(CustomDataset):
    ...
    def __init__(self, **kwargs):
        ...
        self.loader = PRWLoader(dataset_root, self.split)
        super(PRW, self).__init__(**kwargs)
        print("Loaded prw dataset (split=%s) from %s" %
              (self.split, dataset_root))
    ...
    def evaluate(self, results, metric='mAP', logger=None, proposal_nums=(100, 300, 1000), iou_thr=0.5,
        ...
        print("\n")
        import torch
        torch.save(results, "inf_result_prw.pth")
        gallery_det = []
        gallery_feat = []
        probe_feat = []
        for each in results[:-2057]:
        ...
        self.evaluate_search(gallery_det, gallery_feat,
                             probe_feat, gallery_size=gallery_size)
    ...
    def evaluate_search(
        ...
        with open(osp.join(self.loader.dataset_root, "query_info.txt")) as f:
            lines = f.readlines()
            lines = [each.strip().split() for each in lines]
        ...
        name_to_anno = dict()
        for gallery_imname in self.loader.images:
            anno = self.loader.load_annotations(gallery_imname)
            name_to_anno[gallery_imname] = anno
        not_det_cnt = 0
        for i, line in enumerate(lines):
            ...
            # import ipdb; ipdb.set_trace()
            probe_imname = line[5] + ".jpg"
            probe_id = int(line[0])
            probe_roi = np.asarray(list(map(float, line[1:5])))
            probe_roi[2:] += probe_roi[:2]
            probe_gt = []
            tested = set([probe_imname])
            # 1. Go through the gallery samples defined by the protocol
            for item in self.loader.images:
                gallery_imname = str(item)
                # some contain the probe (gt not empty), some not
                anno = name_to_anno[gallery_imname]
                gt = anno[:, 1:][anno[:, 0] == probe_id].squeeze()
                ...
                if gt.size > 0:
                    ...
                    gt[2:] += gt[:2]
                    probe_gt.append(
                        {"img": str(gallery_imname), "roi": list(map(float, list(gt)))})
            ...
            # 2. Go through the remaining gallery images if using full set (not necessary for prw)
        ...
        import json
        with open("results_prw.json", "w") as f:
            json.dump(ret, f)
        ...
        print("{} gallery_imname not in name_to_det_feat".format(not_det_cnt))


if __name__ == '__main__':
    dataset = PRW(split="test", dataset_root="/home/chenzhicheng/Code/mmps/data/prw", ann_file=None, pipeline=[
    ...
    # import ipdb; ipdb.set_trace()
    import torch
    result = torch.load("/home/chenzhicheng/Code/mmps/inf_result.pth")
    import cProfile
    # cProfile.run("dataset.evaluate(result)")
